...
def produceV2results(genres1, topSongs1, genres2, topSongs2, headers):
    weightingsDict = {
        'simTopSongs': 0.2,
        'simEras': 0.2,
        'simCharacteristics': 0.3,
        'simGenres': 0.3
    }
    # Run each similarity function
    topSongsScore = simTopSongs(topSongs1, topSongs2)
    erasScore = simEras(topSongs1, topSongs2, headers)
    characteristicsScore = simCharacteristics(topSongs1, topSongs2, headers)
    genresScore = simGenres(genres1, genres2)
